...
const Dashboard = () => {
    ...
    // Eventos de ejemplo para cada día
    const events = {
        '2025-08-20': [
            { time: '5:00 AM', event: 'Examen de Anatomía Dental' },
            { time: '6:00 AM', event: 'Revisión de tarea de Periodoncia' },
            { time: '7:00 AM', event: 'Examen de Endodoncia' },
        ],
        '2025-08-21': [
            { time: '5:00 AM', event: 'Examen de Cirugía Oral' },
            { time: '6:00 AM', event: 'Estudio de casos de Periodoncia' },
        ],
        '2025-08-22': [
            { time: '10:00 AM', event: 'Revisión de notas de Cirugía Oral' },
            { time: '2:00 PM', event: 'Entrega de tareas de Endodoncia' },
        ],
    };
            ...
            <div className="flex-1 flex flex-col">
                ...
                <main className="flex-1 p-8 overflow-y-auto">
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-lg">
                        <h2 className="text-xl font-bold text-gray-800 mb-4">Calendario: Semana 15</h2>
                        <table className="w-full table-auto border-collapse text-sm">
                            <thead>
                                <tr>
                                    <th className="p-4 bg-gray-100">Hora</th>
                                    <th className="p-4 bg-gray-100">Dom</th>
                                    <th className="p-4 bg-gray-100">Lun</th>
                                    <th className="p-4 bg-gray-100">Mar</th>
                                    <th className="p-4 bg-gray-100">Mié</th>
                                    <th className="p-4 bg-gray-100">Jue</th>
                                    <th className="p-4 bg-gray-100">Vie</th>
                                    <th className="p-4 bg-gray-100">Sáb</th>
                                </tr>
                            </thead>
                            <tbody>
                                {['5:00 AM', '6:00 AM', '7:00 AM'].map((hour, idx) => (
                                    <tr key={idx}>
                                        <td className="border px-4 py-2 bg-gray-50">{hour}</td>
                                        <td className="border px-4 py-2"></td>
                                        <td className="border px-4 py-2">
                                            {events['2025-08-20']?.find(event => event.time === hour)?.event}
                                        </td>
                                        <td className="border px-4 py-2">
                                            {events['2025-08-21']?.find(event => event.time === hour)?.event}
                                        </td>
                                        <td className="border px-4 py-2">
                                            {events['2025-08-22']?.find(event => event.time === hour)?.event}
                                        </td>
                                        <td className="border px-4 py-2"></td>
                                        <td className="border px-4 py-2"></td>
                                        <td className="border px-4 py-2"></td>
